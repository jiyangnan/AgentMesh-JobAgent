...
16. When 51Job send returns `delivery_verification_indeterminate`, `retryable=true` and `request_preserved=true`, immediately run its exact `next_suggested` command with the same preview and authorization. The CLI reconciles clicked jobs from current-site evidence without clicking them again and continues the remaining authorized jobs. When it returns `completion_state=completed_with_unresolved`, stop sending, report cumulative delivered, unavailable and unresolved counts separately, and continue with the returned audit command. Do not rebuild the round, repeat Discover, clear state, manually retry a pending card or count unresolved as delivered/failed.
17. `jobagent init` returns `workbench_url=https://agentmesh360.com/workbench/` for the first-run workbench handoff. An existing account may instead receive top-level `announcements` once after a successful account-verified command. When `id=jobagent_workbench_launch_202608`, show a non-blocking information card if the current host interface supports it, or a concise localized message with the workbench URL. Continue the original `next_suggested` immediately; do not request acknowledgement, rerun `init`, or repeat the announcement.
...
> 请打开 https://agentmesh360.com/app/ 免费注册或登录，在个人中心生成 AgentMesh360 全平台通用 API Key。拿到 Key 后，请回到当前这段 Agent 对话继续配置：可在你信任的私密对话里提供 Key，或在自己的终端运行 `jobagent init --key <你的API_Key>`，然后回来回复“我已配置 API Key，请继续 Job Agent 设置。”我会检查环境和账户，再引导你准备简历与确认目标城市。注册和生成 Key 免费，云端能力按账户可用额度使用。请不要把 Key 发到公开群聊或 Issue。

After the API Key is configured, run `jobagent doctor env`. Treat `environment_healthy` as the environment result and `workflow.ready` as execution readiness; do not reinterpret one as the other. If `cloud_access.usable=true`, tell the user which balance source is active and execute the top-level `next_suggested` when no user action is pending; `signup_trial_active` is a grandfathered entitlement and explicitly means no paid pass is required. Do not inspect or block on the dashboard's pass-purchase status. Ask the user to purchase only when the CLI returns `cloud_access.reason=insufficient_credits` with `paid_pass_required=true`, or a real cloud command returns `insufficient_credits`.
...
the same way. `jobagent resume status --id <resume-id>` reads a single online
resume's confirmation metadata without exposing its draft, body or personal information.
...
同一受管 Chrome 中可能同时保留智联首页与旧城市结果页。Agent 不要自行选择或关闭页签，也不要把旧城市结果当作本轮证据。CLI 会在可见城市动作前后比较不可逆 target 状态指纹，只接管唯一发生变化的官方智联 target，并再次验证目标城市、原始可读查询与真实结果状态；若多个 target 同时变化或证据不足，会保留同一请求并安全停止，不会重复收费。
...
51Job submission is verified on the current `we.51job.com` surface. If a click was observed but final proof is still insufficient, the CLI records `delivery_indeterminate`, continues other authorized jobs, preserves the same preview and authorization, and returns an exact zero-additional-credit reconciliation command. Run that command only while it remains retryable; it inspects pending jobs before any decision and never repeats their apply click. Reconciliation is bounded to two attempts or 24 hours. A job that still lacks sufficient proof becomes terminal `delivery_unresolved`, remains distinct from delivered and failed, and no longer blocks the batch from reaching audit. Do not use the legacy application-history login page as proof that the current 51Job session is logged out.
