"""Boss DOM selectors used for visible actions and bounded search recovery."""
...
BOSS_SELECTOR_VERSION = "2026-07-13.0"


def build_boss_snapshot_script(limit: int = 15) -> str:
    """Extract visible search cards after the direct API rejects the environment."""
    ...
    (function(){{
      ...
      const nodes = Array.from(document.querySelectorAll('.job-card-box, .job-card-wrapper'));
      ...
      for (const card of nodes) {{
        ...
        cards.push({{
          ...
          skills: tags.slice(2),
          jobUrl,
          source: 'boss_search_dom_fallback',
          selectorVersion,
