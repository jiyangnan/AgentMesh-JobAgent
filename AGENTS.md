...
- Installation requests include the setup handoff. Read the final `onboarding_handoff` or run the read-only `jobagent onboarding` after every user-requested install/reinstall/update. Show the current user prompt with the account URL, how to return to the same Agent conversation after obtaining/configuring a Key, and what the Agent will do next. Never end with only installation/version success. Existing credentials require `jobagent doctor env`, not another Key; successful `init` also continues to doctor. Respect user-action and account-recovery boundaries. Install alone does not authorize paid analysis, a new round or delivery. See [installation handoff](docs/agent-onboarding.md#installation-to-account-handoff).
...
- A host that exposes an app reference without a stable window ID can use the returned `app_scoped_window` branch. This verifies the selected profile/account context afresh, not persistent physical-window identity. Read current native state before every UI action; task-permitted window/tab selection may prepare the target, followed by another observation. Collection, job/receipt inspection and recruiting actions still require the bound profile, official page and account. Pause on unresolved window selection; titles and snapshot indices are not stable IDs. Never omit app-scoped evidence or infer missing capability solely from a missing window ID.
- Run the exact `work begin` command only when the current CLI response offers it, before an allowed action, and submit only actual UI observations with its nonce and binding. `reconcile_only` permits receipt inspection, never another send or submit click. Handle technical uncertainty through the returned recovery protocol; relay the user handoff only when `requires_user_action=true`. Do not fall back to CDP, browser JavaScript, hidden site APIs or another driver.
- For an eligible read-only collection failure, follow the returned `work recover` after one explicit confirmation covering closure of the failed task and verification of the same Chrome profile/platform account. Preserve the round, request, Discover, completed pages and candidates; delivery work cannot use this recovery. The agent inspects actual windows and tabs: a changed title or foreground tab alone does not prove the window was lost, and an old title must not be copied as fresh evidence. Follow the [onboarding recovery sequence](docs/agent-onboarding.md#codex-native-execution-first) for older clients; do not send an unknown new command to the old parser or reset observation attempts by repeating recovery.
- Recovery material errors preserve the original request and binding. Follow the typed cause and original recovery command; never substitute a newer resume into an old signed plan. A missing binding may be restored only from its verified signed snapshot after current material and successful browser evidence agree. If the CLI reports `recovery_requires_new_round=true`, obtain explicit confirmation to finish the old round before serial confirmed skips and a new resume/role/city selection. Technical recovery consent alone does not authorize that change. When `recovery_receipt_saved=true`, do not repeat browser actions or create another receipt; follow the returned continuation. See [material recovery](docs/agent-onboarding.md#native-recovery-material).
...
- New Boss greeting work requires a read-only check of the existing exact job conversation in the official message center before send. Follow that task; do not send from a detail popup. Pending messages remain uncertain and terminal unresolved sends are never reopened. Existing issued work keeps its original permission and receipt contract.
- For an explicit greeting refresh on a never-authorized Boss/Liepin list, use the existing preview/review command with `--refresh-greetings`; do not rewrite cloud text. Preserve candidates, choices and history, then show the full new preview for fresh confirmation. Authorized, cancelled or attempted lists are ineligible; unresolved sends remain terminal.
